...
The shared visual-route configuration currently contains **101 representative routes**.
...
With the current 101-route representative contract, an `all` capture produces:

```text
101 desktop full-page PNGs
101 mobile full-page PNGs
202 full-page PNGs total
```
...
The workflow is separate from the normal repository gate so that data-only maintenance does not always generate two hundred two PNGs. A data change that materially alters page length, density, maps, or empty states should trigger the visual workflow by updating the representative contract.
