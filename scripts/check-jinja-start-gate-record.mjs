...
function deploymentConfigFiles(directory) {
  ...
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    if ([".git", "node_modules", ".artifacts", ".release-candidate"].includes(entry.name)) {
      continue;
    }
    ...
    if (/^(?:wrangler.*\.(?:jsonc|toml)|.*worker.*\.jsonc)$/iu.test(entry.name)) {
...
assert(
  !fs.existsSync(path.join(repositoryRoot, "apps", "jinja")),
  "apps/jinja exists before the Jinja start gate has passed",
);
assert(
...
assert(
  packageJson.scripts?.["gate:matsuri:repository"]?.includes(
...
for (const relation of dataset.relations) {
  ...
  if (source.entity_type === "shrine" && matsuriSpecialistTypes.has(target.entity_type)) {
    candidateIds.add(source.id);
  }
  if (target.entity_type === "shrine" && matsuriSpecialistTypes.has(source.entity_type)) {
...
const placeReferences = candidates.reduce(
  (total, entity) =>
    total + unique([entity.primary_place_id, ...(entity.default_place_ids ?? [])]).length,
  0,
);
...
for (const [key, value] of Object.entries(observed)) {
  assert(
    record.seed_baseline?.[key] === value,
    `Jinja seed baseline ${key} expected ${value}, received ${String(record.seed_baseline?.[key])}`,
  );
}
...
console.log(
  `Jinja start gate remains correctly blocked: ${candidates.length} relation-backed seed(s), ${identityEvidence.length} direct identity Evidence record(s), ${placeReferences} Place reference(s), ${approvedStateSnapshots.length} approved shrine State Snapshot(s), ${candidatesWithOfficialUrl.length} seed(s) with official URLs.`,
);
