...
if (requireLockfile && !fs.existsSync(lockfilePath)) {
...
console.log(
  `Node dependency reproducibility guard passed: ${uniquePackageFiles.length} package manifests, ${externalDependencyCount} exact external dependencies, ${workspaceDependencyCount} workspace dependencies${requireLockfile ? ", pnpm-lock.yaml present" : ""}.`,
);
